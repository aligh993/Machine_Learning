...
Data_Set_Learning(:,Pc_number+1)=WDBC(:,trait +1);
number_of_class=max(WDBC(:,trait+1));
trait = Pc_number;
Y=Pc_number;
...
%         zx=count*(Number_Of_Circles)-sumx;
%        while a<zx
%             mlm   =randi(Size_Train);
%             nln=Train_Set(mlm,trait+1);
%              if  ((mark_train(mlm)==0) && (mark_trait(nln)<count))
%                  a = a+1;
...
%         end
center_num=20;
  Center=zeros(center_num,trait+1);
mark_train=zeros(Size_Train);
for cb=1:center_num
     mlm   =randi(Size_Train);
    if  mark_train(mlm)==0 
    mark_train(mlm)=0;
    Center(cb,:)=Train_Set(mlm,:);
      end
end
           MaxDistance=0;
    zx=center_num;
