...
Data_Set_Learning(:,Pc_number+1)=Glass(:,trait +1);
number_of_class=max(Glass(:,trait+1));
trait = Pc_number;
Y=Pc_number;
...
for fold = 1:N_Fold_Cross_Validation
 ...
 Top=Bottom + Size_Test;
        ...
        Test_Set=Data_Set_Learning((Bottom+1):(Top),:);
            tr1=Data_Set_Learning(1:Bottom,Y+1);
        tr2=Data_Set_Learning((Top+1):end,Y+1);
        Train_Target=[tr1;tr2];
        Test_Target=Data_Set_Learning((Bottom+1):(Top),Y+1);
